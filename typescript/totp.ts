...
const ALGORITMOS: Record<string, AlgoritmoTotp> = {
  SHA1: 'SHA-1',
  SHA256: 'SHA-256',
  SHA512: 'SHA-512'
};
...
export function base32ABytes(texto: string): Uint8Array {
  ...
  if (!limpio) throw new TotpInvalido('Falta el secreto del código de verificación.');
  ...
  for (const c of limpio) {
    ...
    if (valor < 0) {
      throw new TotpInvalido(
        `El secreto tiene un carácter que no vale («${c}»). Solo lleva letras y los números del 2 al 7.`
      );
    }
...
function entero(texto: string | undefined, porDefecto: number, que: string): number {
...
export function leerTotp(texto: string): ConfigTotp {
  ...
  if (url.host.toLowerCase() !== 'totp') {
    throw new TotpInvalido(
      'Este código es por contador (HOTP), no por tiempo. PassRod solo admite los de tiempo (TOTP), que son los que usan casi todos los sitios.'
    );
  }
...
export function segundosRestantesTotp(config: ConfigTotp, ahoraMs = Date.now()): number {
...
export async function codigoTotp(config: ConfigTotp, ahoraMs = Date.now()): Promise<string> {
  const sutil = globalThis.crypto?.subtle;
  if (!sutil) throw new Error('Este entorno no tiene WebCrypto para calcular el código.');
  ...
  const clave = await sutil.importKey(
    'raw',
    config.secreto,
